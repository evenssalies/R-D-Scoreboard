...
L'*EU R&D Investment Scoreboard* (*Scoreboard R&D* ou *Scoreboard* par la suite) est une enquête d'entreprises mondiales du secteur privé, qui financent des activités de R&D.[^1] L'enquête est menée chaque année. L'enquête est <ins>non-exhaustive</ins> (voir ci-dessous). Le *Scoreboard* inclut plusieurs variables : le montant de R&D financée (dépenses comptabilisées en frais plus celles activées), le nom de l'entreprise, le pays du siège, l'année d'observation, le secteur NACE (rév. 2),[^2] etc. C'est un panel. Chaque année, la taille de l'échantillon est fixée à l'avance (1000 entrprises de l'Union européenne, 700 américaines, etc.).

### L'exhaustivité de l'enquête

Le *Scoreboard* n'est pas une enquête exhaustive pour plusieurs raisons :
